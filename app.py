...
try:
    ...
    print("⚠️ File database.py atau psycopg2 tidak ditemukan. Menggunakan Mock Functions.")
...
# ==========================================================
# FUNGSI OPTIMASI VEKTORISASI
# ==========================================================
def jalankan_review_data_optimized(df_asli, df_ref=None, nama_file="", session_state=None):
    if session_state is None: session_state = {}
    if df_asli.empty: return pd.DataFrame()
    ...
    if col_ruj: df['is_rujuk_tes'] = df[col_ruj].astype(str).str.replace('.', ',', regex=False).str.contains(r'\b2\b', regex=True, na=False)
    ...
    df['total_log_keseluruhan_klien'] = df.groupby('ssr_id_key')[total_log_cols].transform('sum').sum(axis=1)

    list_error_dfs = []
    keywords_aktif = session_state.get('medsoc_keywords', [])
        ...
        if mask.any():
            ...
            if is_konfirmasi:
                ...
                for idx, row in err_df.iterrows():
                    if row['key_db'] in dict_justifikasi and not dict_revisi.get(row['key_db'], False): continue
    ...
    for col in required_cols:
        if col not in final_df.columns: final_df[col] = "" if col in ["Justifikasi", "validasi hasil review"] else False
    return final_df[required_cols]

# ==========================================================
# FUNGSI DATABASE NEON
# ==========================================================
        ...
        with conn.cursor() as cur:
            for i in range(0, len(list_log_db), chunk_size):
                ...
                extras.execute_batch(
                    ...
                    DO UPDATE SET is_revisi = EXCLUDED.is_revisi, Justifikasi = EXCLUDED.Justifikasi, updated_at = NOW()
                    """,
                    [tuple(row) for row in chunk], page_size=1000
                )
        conn.commit()
        return True
...
# ==========================================================
# INISIALISASI APP & TEMA
# ==========================================================
app = dash.Dash(__name__, external_stylesheets=[dbc.themes.SLATE])
server = app.server  # WAJIB ADA UNTUK DEPLOYMENT (Gunicorn / Hugging Face)

CUSTOM_CSS = """
    body { background-color: #0f172a !important; font-family: 'Inter', system-ui, -apple-system, sans-serif; color: #e2e8f0; margin: 0; min-height: 100vh; }
    .solid-card { background-color: #1e293b; border: 1px solid #334155; border-radius: 12px; padding: 1.5rem; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1); margin-bottom: 20px; transition: border-color 0.2s ease; }
    .solid-card:hover { border-color: #475569; }
    .metric-value { color: #f8fafc !important; font-weight: 700; font-size: 2.2rem; margin: 0; line-height: 1.2; }
    .metric-label { color: #94a3b8; font-size: 0.85rem; margin-top: 8px; text-transform: uppercase; letter-spacing: 0.5px; font-weight: 600; }
    .text-accent-blue { color: #38bdf8 !important; }
    .text-accent-red { color: #fb7185 !important; }
    .text-accent-green { color: #34d399 !important; }
    .btn-modern-primary { background-color: #0ea5e9 !important; border: none !important; color: #ffffff !important; font-weight: 600 !important; border-radius: 8px !important; padding: 10px 20px !important; transition: all 0.2s ease; }
    .btn-modern-primary:hover { background-color: #0284c7 !important; transform: translateY(-1px); box-shadow: 0 4px 12px rgba(14, 165, 233, 0.25); }
    .upload-zone { border: 2px dashed #475569; border-radius: 12px; padding: 2rem; text-align: center; cursor: pointer; transition: all 0.3s; background-color: rgba(30, 41, 59, 0.5); }
    .upload-zone:hover { border-color: #38bdf8; background-color: rgba(56, 189, 248, 0.05); }
    .dash-table-container { background: transparent !important; border: none !important; }
    .dash-spreadsheet-inner td { background-color: #1e293b !important; color: #e2e8f0 !important; border-bottom: 1px solid #334155 !important; font-size: 0.85rem; padding: 12px 15px !important; }
    .dash-spreadsheet-inner th { background-color: #0f172a !important; color: #94a3b8 !important; border-bottom: 2px solid #334155 !important; font-weight: 600; text-transform: uppercase; font-size: 0.75rem; letter-spacing: 0.5px; }
    .dash-spreadsheet tr:hover td { background-color: #334155 !important; }
    .nav-link { color: #94a3b8 !important; font-weight: 500; margin: 0 15px; text-decoration: none !important; padding: 8px 0 !important; border-bottom: 2px solid transparent; transition: all 0.2s; cursor: pointer;}
    .nav-link:hover { color: #e2e8f0 !important; }
    .nav-link.active { color: #38bdf8 !important; border-bottom-color: #38bdf8; }
    .main-content { padding-top: 100px; padding-left: 2.5%; padding-right: 2.5%; max-width: 1650px; margin: 0 auto; }
    .section-title { color: #f8fafc; font-weight: 600; margin-bottom: 1.5rem; font-size: 1.25rem; }
"""
...
# ==========================================================
app.layout = html.Div([
    # STORE UNTUK MENYIMPAN DATA DI MEMORI BROWSER (AMAN UNTUK HF SPACES)
    dcc.Store(id='store-df-raw', storage_type='session'),
    dcc.Store(id='store-df-errors', storage_type='session'),
    dcc.Store(id='store-riwayat', storage_type='session', data=[]),
    ...
    html.Div([
        dbc.Row([
            ...
            dbc.Col(html.Div([
                html.A("Dashboard", className="nav-link active", id="nav-dashboard"),
                html.A("Riwayat", className="nav-link", id="nav-riwayat"),
                html.A("Laporan", className="nav-link", id="nav-laporan"),
    ...
    ], style={'position': 'fixed', 'top': '0', 'left': '0', 'right': '0', 'height': '70px', 'zIndex': '1000', 'backgroundColor': '#0f172a', 'borderBottom': '1px solid #1e293b', 'padding': '0 2.5%'}),
    ...
    html.Div(className="main-content", children=[
        html.Div(id="tab-content-container", children=[
            ...
            html.Div(id="tab-dashboard", children=[
                ...
                dbc.Row([
                    dbc.Col(dbc.Card([html.H2(id="metric-total-data", className="metric-value text-accent-blue", children="0"), html.P("Total Data Diproses", className="metric-label")], className="solid-card text-center border-0"), width=4),
                    dbc.Col(dbc.Card([html.H2(id="metric-temuan", className="metric-value text-accent-red", children="0"), html.P("Temuan Error", className="metric-label")], className="solid-card text-center border-0"), width=4),
                    dbc.Col(dbc.Card([html.H2(id="metric-akurasi", className="metric-value text-accent-green", children="0%"), html.P("Tingkat Akurasi", className="metric-label")], className="solid-card text-center border-0"), width=4),
                ], className="mb-4 g-3"),

                html.Div([
                    html.H4("Hasil Review Detail", className="section-title"),
                    dcc.Loading(
                        id="loading-table", type="circle", color="#38bdf8",
                        children=dash_table.DataTable(
                            id='tabel-detail', editable=True, page_size=20,
            ...
            html.Div(id="tab-riwayat", style={'display': 'none'}, children=[
                ...
                html.Div([
                    dash_table.DataTable(
                        id='tabel-riwayat',
                        columns=[
                            {"name": "Waktu", "id": "waktu"}, {"name": "Periode", "id": "periode"},
                            {"name": "Total Data", "id": "total_data"}, {"name": "Temuan", "id": "temuan"},
                            {"name": "Akurasi", "id": "akurasi"}, {"name": "Status Arsip", "id": "status_arsip"}
                        ],
                        page_size=15, style_table={'overflowX': 'auto', 'borderRadius': '12px'}
                    )
                ], className="solid-card"),
                dbc.Alert("ℹ️ Riwayat validasi tersimpan otomatis setiap kali tombol 'Jalankan Validasi' ditekan.", 
                          color="info", className="mt-3", style={'backgroundColor': 'rgba(56, 189, 248, 0.1)', 'borderColor': 'rgba(56, 189, 248, 0.3)', 'color': '#38bdf8'})
            ]),
            ...
            html.Div(id="tab-laporan", style={'display': 'none'}, children=[
                ...
                dbc.Row([
                    dbc.Col(dbc.Card([
                        ...
                        html.Div(id="download-excel-trigger", className="d-none"), # Placeholder
                    ...
                    dbc.Col(dbc.Card([
                        ...
                        html.Div("Fitur PDF akan segera hadir.", style={'color': '#fbbf24', 'marginTop': '10px'}),
                        dbc.Button("📄 Download PDF", id="btn-dl-pdf", className="btn-modern-primary w-100 mt-3", disabled=True)
                    ], className="solid-card"), width=6)
                ])
            ])
        ])
...
# ==========================================================
@callback(Output('store-df-raw', 'data'), Output('status-upload-penjangkauan', 'children'), 
          Input('upload-penjangkauan', 'contents'), State('upload-penjangkauan', 'filename'))
def update_upload_status(contents, filename):
    if contents:
        content_type, content_string = contents.split(',')
        decoded = base64.b64decode(content_string)
        try:
            if 'csv' in filename: df = pd.read_csv(io.StringIO(decoded.decode('utf-8')), low_memory=False)
            else: df = pd.read_excel(io.BytesIO(decoded))
            
            # Kembalikan JSON string agar aman disimpan di browser
            return df.to_json(date_format='iso', orient='split'), f"✅ {filename} berhasil dimuat ({len(df):,} baris)"
        except Exception as e:
...
@callback(
    ...
    Output('store-df-errors', 'data'), Output('store-riwayat', 'data'),
    Input('btn-jalankan', 'n_clicks'), 
    State('store-df-raw', 'data'), State('store-riwayat', 'data'),
    prevent_initial_call=True
)
def run_validation(n_clicks, raw_json, riwayat_data):
    if not raw_json:
        return "0", "0", "0%", [], [], "⚠️ Silakan upload file penjangkauan terlebih dahulu!", no_update, no_update

    df_raw = pd.read_json(raw_json, orient='split')
    if df_raw.empty: return "0", "0", "0%", [], [], "⚠️ Data kosong", no_update, no_update

    df_errors = jalankan_review_data_optimized(df_raw)
    ...
    cols = [{"name": i, "id": i, "editable": (i in ['Pilih', 'Justifikasi'])} for i in df_errors.columns] if df_errors is not None and not df_errors.empty else []
    data = df_errors.to_dict('records') if df_errors is not None and not df_errors.empty else []
    
    errors_json = df_errors.to_json(date_format='iso', orient='split') if df_errors is not None and not df_errors.empty else None

    return f"{tot_data:,}", f"{tot_err:,}", f"{akurasi:.1f}%", data, cols, "✅ Validasi selesai! Silakan review tabel di bawah.", errors_json, riwayat

# ==========================================================
# CALLBACKS: SIMPAN KE DATABASE
# ==========================================================
@callback(Output('status-simpan', 'children', allow_duplicate=True), 
          Input('btn-simpan', 'n_clicks'), State('tabel-detail', 'data'), prevent_initial_call=True)
def save_to_db(n_clicks, table_data):
    ...
    if list_log:
        ...
        return "❌ Gagal menyimpan ke database (Cek koneksi internet/server)."
    return "ℹ️ Centang 'Pilih' atau isi 'Justifikasi' untuk menyimpan."

# ==========================================================
# CALLBACKS: UPDATE TABEL RIWAYAT
# ==========================================================
@callback(Output('tabel-riwayat', 'data'), Input('store-riwayat', 'data'))
def update_riwayat_table(riwayat_data):
    return riwayat_data or []

# ==========================================================
# CALLBACKS: DOWNLOAD EXCEL (PERBAIKAN BUG)
# ==========================================================
@callback(Output('download-excel', 'data'), Input('btn-dl-excel', 'n_clicks'), State('store-df-errors', 'data'), prevent_initial_call=True)
def download_excel(n_clicks, errors_json):
    if not errors_json: return None
    df_errors = pd.read_json(errors_json, orient='split')
    return dcc.send_data_frame(df_errors.to_excel, "laporan_rekap_kesalahan.xlsx", index=False)
